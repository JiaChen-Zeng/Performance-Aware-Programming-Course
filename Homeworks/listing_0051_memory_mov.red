...
#system [
    ...
    set-memory86: func [
        addr [integer!] wide [logic!] value [integer!]
    ] [
        either wide [set-int16 memory + addr value]
                    [memory/addr: as-byte value]
    ]

    get-memory86: func [
        addr [integer!] wide [logic!]
        return: [integer!]
    ] [
        return either wide [decode-int16 memory + addr]
                           [as-integer memory/addr]
    ]
    ...
    decode-r_m-t_f-register: func [
        ...
        reg-index [integer!]
        res [resource!] rreg [resource-reg!] mreg [resource-mem!] ret [integer!]
    ] [
        opstr: switch op [
            #"^(88)" [OPSTR_MOV]
            #"^(00)" [OPSTR_ADD]
        ...
        switch res/type [
            ...
            RT_MEM [
                mreg: as resource-mem! res
                if op = #"^(88)" [
                    either direction [
                        ret: get-memory86 mreg/addr wide
                        set-register reg-index wide ret
                    ] [
                        ret: get-register reg-index wide
                        set-memory86 mreg/addr wide ret
                    ]
                ]
            ]
        ]
...
bin: read/binary %../computer_enhance/perfaware/part1/listing_0051_memory_mov
