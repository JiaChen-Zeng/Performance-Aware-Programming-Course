...
#system [
    ...
    decode-r_m-t_f-register: func [
        ...
        reg-index [integer!]
        res [resource!] rreg [resource-reg!]
    ] [
        str: switch op [
            #"^(88)" ["mov "]
            #"^(00)" ["add "]
        ...
        ; Decode reg
        reg-index: 0
        str: decode-reg byte 3 wide :reg-index
        ...
        ; Sim only for registers
        if res/type = RT_REG [
            rreg: as resource-reg! res
            either direction [
                set-register reg-index wide get-register rreg/index wide
            ] [
                set-register rreg/index wide get-register reg-index wide
            ]
        ]
...
; bin: read/binary %../computer_enhance/perfaware/part1/listing_0043_immediate_movs
