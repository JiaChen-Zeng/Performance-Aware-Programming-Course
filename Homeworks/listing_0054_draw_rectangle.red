...
view-image: func [
    ...
    pos [pair!] color [tuple!]
] [
    size: to-pair width height
    draw-content: reduce ['pen 'off]
    pos: 0x0
    color: 0.0.0.0
    loop width [
        loop height [
            color/1: get-memory-red addr + (pos/y * width + pos/x * 4)
            color/2: get-memory-red addr + (pos/y * width + pos/x * 4 + 1)
            color/3: get-memory-red addr + (pos/y * width + pos/x * 4 + 2)
            color/4: complement get-memory-red addr + (pos/y * width + pos/x * 4 + 3)
            repend draw-content ['fill-pen color 'box pos (pos + 1x1)]
            pos/x: pos/x + 1
        ]
        pos/x: 0
        pos/y: pos/y + 1
    ]
    view [base size draw draw-content]
]
